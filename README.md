...
2. ダッシュボードの **「Install Server」** をクリック（コンソール画面に自動切り替えされ、パルワールドサーバーのダウンロードが始まります）
   ※ SteamCMDは `install.sh` 実行時に自動インストール済みです！
3. インストール完了後、ダッシュボードの **「▶️ Start」** をクリックしてサーバーを起動！
4. **ゲームから接続**:
