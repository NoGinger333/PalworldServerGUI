...
1. **`bash install.sh` を実行するだけで** Python環境、SteamCMD、**パルワールド専用サーバー本体がすべて全自動でインストールされます！**
2. `./run.sh` を実行してWeb UIを起動します（`http://localhost:5000`）
3. デフォルトパスワード `admin` でログイン！
4. ダッシュボードの **「▶️ Start」** ボタンをクリックして即サーバー稼働！
5. **ゲームから接続**:
